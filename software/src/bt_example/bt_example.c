...
void IntMode(){
		...
		BT2_SetupInterruptSystem(&myDevice, 0, XPAR_MICROBLAZE_0_AXI_INTC_PMODBT2_0_BT2_UART_INTERRUPT_INTR, recvd, sent, timeout);
#else
		BT2_SetupInterruptSystem(&myDevice, XPAR_PS7_SCUGIC_0_DEVICE_ID, XPAR_MICROBLAZE_0_AXI_INTC_PMODBT2_0_BT2_UART_INTERRUPT_INTR, recvd, sent, timeout);//Setup interrupts, Zynq
#endif
#endif
		XUartNs550_Recv(&myDevice.BT2Uart, (u8*)myDevice.recv, 600);//Wait for new sentence
